...
## Usage
`/generate-android-test-lite <test-name>` — e.g. `/generate-android-test-lite email-sign-up-flow`

## How this differs from /generate-android-test
Step 5 uses android-cli instead of raw ADB:
- `take_annotated_screenshot` + `resolve_coordinates` replace `dump_ui` + manual bounds math
- `get_layout(diff=True)` replaces full re-dumps after dropdowns
- `take_annotated_screenshot` doubles as screen-transition confirmation
...
#### 5.2 — Annotate once per screen, resolve all coordinates

When you arrive at a new screen:
1. Call `mcp__android-adb__take_annotated_screenshot` **once** — every visible element is numbered.
2. Identify label numbers for **every interaction you need on that screen**.
3. For each, call `mcp__android-adb__resolve_coordinates("input tap #N")` to get exact `(x, y)`.
4. Record those coordinates in the selector map (Step 5.6).
5. Fire all taps and inputs back-to-back — no intermediate screenshots or layout calls.

The only exceptions requiring a re-annotate:
- A **dropdown or modal opened** (new elements appeared).
- A step says **"Wait"** (you navigated to the next screen).
...
**Never use `press_key BACK` to dismiss the keyboard** if the app uses BACK for screen navigation — pressing BACK would navigate away. Tap a non-interactive element in a fixed header area instead.

#### 5.5 — Dropdowns

Tap the dropdown trigger → call `mcp__android-adb__get_layout(diff=True)` to see only new elements → resolve or compute item coordinate → tap item. Do **not** re-check after selection.
...
2. `resource-id` present in layout JSON (and no testTag was added) → `onNodeWithTag(id.substringAfterLast('/'))`
...
#### 5.7 — Fallback when annotation misses an element
